...
# TOKENni olish (Railway’da ham Environment Variable nomi TOKEN bo‘lishi kerak)
TOKEN = os.getenv("TOKEN")

if not TOKEN:
    raise ValueError("❌ TOKEN Environment Variable topilmadi! .env fayl va Railway sozlamalarini tekshiring.")
...
# Xabarlarni relay qilish
@bot.message_handler(content_types=['text', 'photo', 'video', 'document', 'audio', 'voice', 'sticker'])
def relay_message(message):
    user = message.from_user.id
    if user not in chat_pairs:
        bot.reply_to(message, "❗️ Вы не участвуете в разговоре. Используйте команду /find")
        return
    partner = chat_pairs[user]
    try:
